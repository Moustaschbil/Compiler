...
void parse_args(int argc, char ** argv) {
    ...
    infile = argv[1];
    int operande;
    while((operande = getopt(argc, argv, ":bo:t:r:svh")) != -1)
    {
            switch(operande)
            {
                    case 'b': printf(   "####       ####   ##  ####         ##    ##       ########         ########    \n"
                                        ...
                                        "Compilateur réalisé par Walid GHETTAS & Théo PALLIER\n");
                                EXIT_SUCCESS;
                                break;
                    case 'o': outfile = optarg;
                                break;
                    case 't': trace_level = atoi(optarg);
                                if(trace_level < 0 || trace_level > 5)
                                        printf("Erreur: niveau de trace non compris entre 0 et 5.\n");
                                break;
                    case 'r': nb_reg = atoi(optarg);
                                if (nb_reg < 4 || nb_reg > DEFAULT_MAX_REGS)
                                        printf("Erreur: nombre de registres non compris entre 4 et 8.\n");
                                break;
                    case 's':   if(stop_after_verif == true)
                                {
                                        printf("Erreur: les options '-s' et '-v' sont incompatibles.\n");
                                        EXIT_FAILURE;
                                }
                                stop_after_syntax = true;
                                break;
                    case 'v':   if(stop_after_syntax == true)
                                {
                                        printf("Erreur: les options '-s' et '-v' sont incompatibles.\n");
                                        EXIT_FAILURE;
                                }
                    ...
                    case 'h': system("echo \e[1mLISTE DES OPTIONS\e[0m");
                ...
                printf("-t <int> : Définit le niveau de trace à utiliser entre 0 et 5 (0 = pas de trace ; 5 = toutes les traces. defaut = 0).\n");
                                ...
                                EXIT_SUCCESS;
                    case '?': printf("Option(s) non reconnue(s). Voir les options disponibles avec l'option '-h'. \n");
                                EXIT_SUCCESS;
                                break;
